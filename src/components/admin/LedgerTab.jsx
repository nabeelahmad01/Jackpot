...
export default function LedgerTab({
  ...
  const swrKey = activeTab === 'pending'
    ? `/api/transactions?status=PENDING,PENDING_COINS&page=${page}&limit=${limit}&search=${encodeURIComponent(debouncedSearch)}`
    : `/api/transactions?page=${historyPage}&limit=${limit}&search=${encodeURIComponent(historyDebouncedSearch)}&status=${historyStatus}&type=${historyType}`;
  ...
  const transactions = activeTab === 'pending'
    ? rawTransactions.filter((t) => !completedActionIds[t.id])
    : rawTransactions;
  ...
  return (
    <section className="admin-section-card" style={{ animation: 'fade-in 0.2s ease-out' }}>
      ...
      {/* Tab Switcher */}
      <div style={{ display: 'flex', gap: '0.75rem', borderBottom: '1px solid rgba(255,255,255,0.05)', paddingBottom: '0.75rem', marginBottom: '1.25rem' }}>
        <button
          onClick={() => setActiveTab('pending')}
          style={{
            background: activeTab === 'pending' ? 'var(--gold-primary)' : 'none',
        ...
        >
          <i className="fa-solid fa-clock"></i> Action Queue (Pending)
        </button>
        <button
          onClick={() => setActiveTab('history')}
          style={{
            background: activeTab === 'history' ? 'var(--gold-primary)' : 'none',
        ...
        >
          <i className="fa-solid fa-magnifying-glass"></i> Transaction Search (All Logs)
        </button>
      </div>
        ...
        <>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.75rem', marginBottom: '1.25rem', padding: '1rem', background: '#0b0d16', borderRadius: '8px' }}>
            <div className="input-wrapper search-wrapper" style={{ flex: 1, minWidth: '240px', background: '#07090f', margin: 0 }}>
              ...
              <input
                type="text"
                placeholder="Search history by player email..."
                value={historySearch}
                ...
                {transactions.length === 0 ? (
                  <tr>
                    <td colSpan="9" className="text-center text-muted" style={{ padding: '2rem' }}>
                      No matching historical transaction logs found.
                    </td>
                  </tr>
                ) : (
